...
function App() {
  const { isEmbedded, notifyViewChange } = useEmbeddedMode();
  ...
  const handleViewChange = (view: 'tools' | 'viewer' | 'editor' | 'batch' | 'analytics' | 'workflows' | 'quality' | 'cloud' | 'help' | 'admin') => {
    setCurrentView(view);
    if (isEmbedded) {
  ...
  const renderCurrentView = () => {
    switch (currentView) {
      case 'viewer':
        return <PDFViewer selectedTool={selectedTool} onBack={() => handleViewChange('tools')} />;
      case 'editor':
        return <PDFEditor onBack={() => handleViewChange('tools')} />;
      case 'batch':
        return <BatchProcessor onBack={() => handleViewChange('tools')} />;
      case 'analytics':
        return <Analytics stats={processingStats} onBack={() => handleViewChange('tools')} />;
      case 'workflows':
        return <WorkflowDesigner onBack={() => handleViewChange('tools')} />;
      case 'quality':
        return <QualityAnalyzer onBack={() => handleViewChange('tools')} />;
      case 'cloud':
        return <CloudConnector onBack={() => handleViewChange('tools')} />;
      case 'help':
        return <HelpSystem onBack={() => handleViewChange('tools')} />;
      case 'admin':
      ...
      default:
        return (
          <div className="space-y-6">
            <AdminDashboard onBack={() => handleViewChange('tools')} />
    ...
    <div className="min-h-screen bg-gray-50">
      {!isEmbedded && (
        ...
        />
          ...
          <main className={`${isEmbedded ? 'flex-1' : 'flex-1'} p-6`}>
            {renderCurrentView()}
          </main>
        </div>
    </div>
